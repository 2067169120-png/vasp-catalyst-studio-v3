...
with **5191 passed and 12 skipped** in 670.53 seconds, with 15 upstream warnings; full-repository Ruff,
`node --check` for 30 first-party JavaScript files, `pip check`, and `git diff --check` passed. Actionlint
was not installed locally and remains a hosted-CI gate. The `vcstudio-4.0.0` wheel contains 52 Web assets
and both locales. The PyInstaller `--full` EXE built from repository-layout commit `9bb6a0d` is
