...
def valid_result(network=None):
        ...
        "points": [
            {
                ...
                "drc": [{
                    "target_species_id": "CO2_g", "step_id": "co_oxidation",
                    "value": 0.9,
                }],
                "dsc": [{
                    "target_species_id": "CO2_g", "step_id": "co_oxidation",
                ...
                "reaction_order": [{
                    "target_species_id": "CO2_g", "species_id": "CO_g",
                    "value": 0.8,
                }],
    ...
    if network.get("network_id") == "catmap-ready-adsorption":
        ...
        point.update({
            "tof": [
                {"species_id": "CO_s", "value": 2.5},
                {"species_id": "O2_s", "value": 1.25},
            ],
            "coverage": [
                {"species_id": "CO_s", "site_type": "s", "value": 0.35},
            ...
            "selectivity": [
                {"species_id": "CO_s", "value": 0.6},
                {"species_id": "O2_s", "value": 0.4},
            ],
            "drc": [
                {
                    "target_species_id": "CO_s", "step_id": "co_adsorption",
                    "value": 0.9,
                },
                {
                    "target_species_id": "O2_s", "step_id": "co_adsorption",
                    "value": -0.2,
                },
                {
                    "target_species_id": "CO_s", "step_id": "o2_adsorption",
                    "value": 0.25,
                },
                {
                    "target_species_id": "O2_s", "step_id": "o2_adsorption",
                    "value": 0.75,
                },
            ],
            "dsc": [
            ...
            "reaction_order": [
                {
                    "target_species_id": "CO_s", "species_id": "CO_g",
                    "value": 0.8,
                },
                {
                    "target_species_id": "O2_s", "species_id": "CO_g",
                    "value": -0.3,
                },
                {
                    "target_species_id": "CO_s", "species_id": "O2_g",
                    "value": 0.2,
                },
                {
                    "target_species_id": "O2_s", "species_id": "O2_g",
                    "value": 0.6,
                },
            ],
            "apparent_activation_energy": [
                {"species_id": "CO_s", "value": 0.7},
                {"species_id": "O2_s", "value": 0.8},
            ],
            "free_energy_diagram": [
...
@pytest.mark.parametrize(
    ("mutate", "code"),
    [
        (lambda n: n["species"][6].update(
            {"frequencies_cm1": [450.0, 300.0, 700.0]}),
         "TRANSITION_STATE_IMAGINARY_MODE_REQUIRED"),
        (lambda n: n["species"][4].update(
            {"frequencies_cm1": [-75.0, 350.0, 1800.0]}),
         "STATIONARY_SPECIES_IMAGINARY_MODE_FORBIDDEN"),
        (lambda n: n["species"][6].update({
            "phase": "adsorbate", "frequencies_cm1": [450.0, 300.0, 700.0],
         }), "TRANSITION_STATE_SPECIES_REQUIRED"),
    ],
)
def test_stationary_point_roles_and_imaginary_modes_fail_closed(mutate, code):
    network = frozen_network()
    mutate(network)
    _freeze(network)
...
def test_small_negative_frequency_is_bounded_numerical_noise():
    network = frozen_network()
    network["species"][0]["frequencies_cm1"][0] = -25.0
    _freeze(network)
...
@pytest.mark.parametrize("species_id", ["CO_s", "CO_g"])
def test_transition_state_side_rejects_non_ts_reaction_species(species_id):
    network = frozen_network()
    network["elementary_steps"][0]["transition_state"][species_id] = 1
    _freeze(network)

    audit = kinetics.audit_network(network)

    assert "TRANSITION_STATE_PHASE_INVALID" in _codes(audit)
    assert audit["machine_pass"] is False
...
def test_expected_adapter_requires_exact_tool_version_identity():
    network = frozen_network()
    result = valid_result(network)
    without_tool_version = {
        "id": "vcstudio.catmap-process-adapter", "version": "3",
        "tool_sha256": "4" * 64,
    }
    with pytest.raises(
            kinetics.KineticsContractError,
            match="expected_adapter must contain"):
        kinetics.import_result(
            result, network, expected_adapter=without_tool_version)

    wrong_tool_version = {
        **without_tool_version, "tool_version": "0.5.0",
    }
    with pytest.raises(
            kinetics.KineticsContractError, match="tool_version mismatch"):
        kinetics.import_result(
            result, network, expected_adapter=wrong_tool_version)


def test_target_scoped_metrics_keep_both_axes_for_multiple_products():
    ...
    assert point["drc"] == [
        {
            "step_id": "co_adsorption", "value": 0.9,
            "target_species_id": "CO_s",
        },
        {
            "step_id": "co_adsorption", "value": -0.2,
            "target_species_id": "O2_s",
        },
        {
            "step_id": "o2_adsorption", "value": 0.25,
            "target_species_id": "CO_s",
        },
        {
            "step_id": "o2_adsorption", "value": 0.75,
            "target_species_id": "O2_s",
        },
    ]
    assert point["dsc"] == [
        {
            "step_id": "co_adsorption", "value": 0.1,
            "target_species_id": "CO_s",
        },
        {
            "step_id": "co_adsorption", "value": 0.4,
            "target_species_id": "O2_s",
        },
        {
            "step_id": "o2_adsorption", "value": -0.1,
            "target_species_id": "CO_s",
        },
        {
            "step_id": "o2_adsorption", "value": 0.6,
            "target_species_id": "O2_s",
        },
    ]
    assert point["reaction_order"] == [
        {
            "species_id": "CO_g", "value": 0.8,
            "target_species_id": "CO_s",
        },
        {
            "species_id": "CO_g", "value": -0.3,
            "target_species_id": "O2_s",
        },
        {
            "species_id": "O2_g", "value": 0.2,
            "target_species_id": "CO_s",
        },
        {
            "species_id": "O2_g", "value": 0.6,
            "target_species_id": "O2_s",
        },
    ]
    assert point["tof"] == [
    ...
    assert point["selectivity"] == [
        {"species_id": "CO_s", "value": 0.6},
        {"species_id": "O2_s", "value": 0.4},
    ]
    assert point["apparent_activation_energy"] == [
        {"species_id": "CO_s", "value": 0.7},
        {"species_id": "O2_s", "value": 0.8},
    ]
    assert point["completeness"]["drc"] == {
        "identity_fields": ["target_species_id", "step_id"],
        "required_records": 4, "observed_records": 4, "unique_records": 4,
        "duplicate_records": 0, "complete": True,
    }
    assert point["completeness"]["reaction_order"] == {
        "identity_fields": ["target_species_id", "species_id"],
        "required_records": 4, "observed_records": 4, "unique_records": 4,
        "duplicate_records": 0, "complete": True,
    }


def test_all_empty_metric_arrays_are_rejected_as_incomplete():
    network = frozen_network()
    result = valid_result(network)
    for metric in (
            "tof", "coverage", "selectivity", "drc", "dsc",
            "reaction_order", "apparent_activation_energy",
            "free_energy_diagram"):
        result["points"][0][metric] = []

    with pytest.raises(
            kinetics.KineticsContractError,
            match=r"points\[0\]\.tof does not completely cover"):
        kinetics.import_result(
            result, network,
            expected_adapter={
                "id": "vcstudio.catmap-process-adapter", "version": "3",
                "tool_version": "0.4.0", "tool_sha256": "4" * 64,
            },
        )
...
@pytest.mark.parametrize(
    "metric",
    [
        "tof", "selectivity", "drc", "dsc", "reaction_order",
        "apparent_activation_energy",
    ],
)
def test_partial_multi_target_metric_coverage_is_rejected(metric):
    network = catmap_ready_network()
    result = valid_result(network)
    result["points"][0][metric].pop()

    with pytest.raises(
            kinetics.KineticsContractError, match="does not completely cover"):
        kinetics.import_result(
            result, network,
            expected_adapter={
                "id": "vcstudio.catmap-process-adapter", "version": "3",
...
def test_duplicate_metric_identity_is_rejected(metric):
    network = frozen_network()
    result = valid_result(network)
    records = result["points"][0][metric]
    records.append(copy.deepcopy(records[0]))

    with pytest.raises(kinetics.KineticsContractError, match="duplicate"):
        kinetics.import_result(
            result, network,
            expected_adapter={
                "id": "vcstudio.catmap-process-adapter", "version": "3",
...
def test_coverage_completeness_requires_every_frozen_site_type():
    network = frozen_network()
    bridge_site = copy.deepcopy(network["species"][3])
    bridge_site.update({"id": "star_b", "sites": {"b": 1}})
    network["species"].append(bridge_site)
    _freeze(network)
    result = valid_result(network)
    result["points"][0]["coverage"].append({
        "species_id": "star_b", "site_type": "b", "value": 0.0,
    })
    ...
    assert normalized["points"][0]["completeness"]["coverage"] == {
        "identity_fields": ["species_id", "site_type"],
        "required_records": 2, "required_site_types": 2,
        "observed_site_types": 2,
        "observed_records": 3, "unique_records": 3,
        "duplicate_records": 0, "complete": True,
    }

    result["points"][0]["coverage"].pop()
    with pytest.raises(
            kinetics.KineticsContractError,
            match="does not cover every frozen site type"):
        kinetics.import_result(
            result, network,
            expected_adapter={
                "id": "vcstudio.catmap-process-adapter", "version": "3",
...
def test_free_energy_completeness_requires_two_distinct_states():
    network = frozen_network()
    result = valid_result(network)
    result["points"][0]["free_energy_diagram"] = [
        result["points"][0]["free_energy_diagram"][0],
    ]

    with pytest.raises(
            kinetics.KineticsContractError, match="at least two distinct"):
        kinetics.import_result(
            result, network,
            expected_adapter={
                "id": "vcstudio.catmap-process-adapter", "version": "3",
...
@pytest.mark.parametrize("metric", ["drc", "dsc", "reaction_order"])
@pytest.mark.parametrize(
    ("target_species_id", "match"),
    [(None, "target_species_id"), ("CO_g", "frozen target products")],
)
def test_target_scoped_metrics_require_a_frozen_target(
        metric, target_species_id, match):
    network = catmap_ready_network()
    result = valid_result(network)
    record = result["points"][0][metric][0]
    if target_species_id is None:
        record.pop("target_species_id")
    else:
        record["target_species_id"] = target_species_id
...
@pytest.mark.parametrize(
    "metric", ["tof", "selectivity", "apparent_activation_energy"])
def test_output_metrics_require_a_frozen_target_product(metric):
    network = frozen_network()
    result = valid_result(network)
    result["points"][0][metric][0]["species_id"] = "CO_g"

    with pytest.raises(
            kinetics.KineticsContractError, match="frozen target products"):
        kinetics.import_result(
            result, network,
            expected_adapter={
                "id": "vcstudio.catmap-process-adapter", "version": "3",
...
@pytest.mark.parametrize("species_id", ["CO_s", "O2_g"])
def test_reaction_order_requires_a_gas_feed_reservoir(species_id):
    network = frozen_network()
    result = valid_result(network)
    result["points"][0]["reaction_order"][0]["species_id"] = species_id

    with pytest.raises(
            kinetics.KineticsContractError, match="gas feed reservoir"):
        kinetics.import_result(
            result, network,
            expected_adapter={
                "id": "vcstudio.catmap-process-adapter", "version": "3",
...
def test_coverage_species_must_be_surface_or_adsorbate():
    network = frozen_network()
    result = valid_result(network)
    result["points"][0]["coverage"][0].update({
        "species_id": "CO_g", "site_type": "s",
    })

    with pytest.raises(
            kinetics.KineticsContractError,
            match="species that cannot carry phase-1 coverage"):
        kinetics.import_result(
            result, network,
            expected_adapter={
                "id": "vcstudio.catmap-process-adapter", "version": "3",
...
def test_coverage_site_must_belong_to_the_reported_species():
    network = frozen_network()
    bridge_site = copy.deepcopy(network["species"][3])
    bridge_site.update({"id": "star_b", "sites": {"b": 1}})
    network["species"].append(bridge_site)
    _freeze(network)
    result = valid_result(network)
    # ``b`` exists globally on star_b, but CO_s is frozen on ``s`` only.
    result["points"][0]["coverage"][0]["site_type"] = "b"

    with pytest.raises(
            kinetics.KineticsContractError,
            match="site type is not assigned to its frozen species"):
        kinetics.import_result(
            result, network,
            expected_adapter={
                "id": "vcstudio.catmap-process-adapter", "version": "3",
...
def test_free_energy_diagram_rejects_unfrozen_safe_state_ids():
    network = frozen_network()
    result = valid_result(network)
    result["points"][0]["free_energy_diagram"][1]["state_id"] = "invented_state"

    with pytest.raises(
            kinetics.KineticsContractError,
            match="frozen species and reserved aggregate states"):
        kinetics.import_result(
            result, network,
            expected_adapter={
                "id": "vcstudio.catmap-process-adapter", "version": "3",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (lambda r: r["adapter"].update({"tool_version": "0.5.0"}),
        ...
        (lambda r: r["points"][0]["drc"][0].update({"step_id": "invented"}),
         "steps outside the frozen network"),
        (lambda r: r["points"][0]["reaction_order"][0].update(
            {"species_id": "invented"}), "species outside the frozen network"),
    ],
)
...
def test_reported_failure_or_large_residual_keeps_result_unavailable():
    network = frozen_network()
    result = valid_result(network)
