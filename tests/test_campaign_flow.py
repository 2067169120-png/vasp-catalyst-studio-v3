...
from vcstudio.campaign import (
    accept_gate, budget, derive, fingerprint, ledger, lock,
...
def test_end_to_end_pilot_then_fanout(tmp_path):
    ...
    states.mark_running(pilot, campaign_dir=cdir)
    states.mark_completed(pilot, campaign_dir=cdir)
    budget.record_actual(cdir, 'pilot-relax', 260.0)
    states.promote_validated(pilot, [{'name': 'converged', 'ok': True},
                                     {'name': 'force<EDIFFG', 'ok': True}], campaign_dir=cdir)
    av = accept_gate({'task_fingerprint_hash': pilot['fingerprint_hash'],
                      'campaign_fingerprint_hash': camp_fp_hash,
                      'required_checks': [{'name': 'lit-value-aligned', 'ok': True}],
                      'energy_eV': -123.45})
    ...
    # 5) 推完矩阵两任务至 accepted
    for t in (m1, m2):
        states.mark_completed(t, campaign_dir=cdir)
        states.promote_validated(t, [{'name': 'converged', 'ok': True}], campaign_dir=cdir)
...
def test_waive_flow_with_real_decision_id(tmp_path):
    ...
    cdir = camp['dir']
    # 人工决策:批准超预算一次
    did = ledger.record_decision(cdir, 'budget-waiver',
                                 '导师批准本次超预算 20 核时', 'user',
                                 context={'task': 'a'})
    v = submit_gate({'estimated_core_hours': 120, 'remaining_budget': 100,
...
def test_downgrade_after_problem_records_and_reblocks(tmp_path):
    ...
    states.mark_completed(t)
    states.promote_validated(t, [{'name': 'x', 'ok': True}])
    states.promote_accepted(t, {'gate': 'accept_gate', 'status': 'pass'}, campaign_dir=cdir)
