...
const view = {
  ...
  points: [{
    ...
    drc: [{ target_species_id: 'TARGET_P', step_id: 's1', value: 40404,
      display: 'SERVER_DRC' }],
    dsc: [{ target_species_id: 'TARGET_P', step_id: 's1', value: 50505,
      display: 'SERVER_DSC' }],
    reaction_order: [{ target_species_id: 'TARGET_P', species_id: 'PERTURBED_A',
      value: 60606, display: 'SERVER_ORDER' }],
...
def test_kinetic_adapter_controls_require_preview_and_explicit_confirmation():
    ...
    assert 'id="aw-kinetics-result-label"' in html
...
def test_invalid_kinetics_upload_receipt_never_enables_result_selection():
    _run_node(
        r"""
const select = element('aw-kinetics-select-result', { disabled: true });
const status = element('aw-kinetics-adapter-status');
const fileInput = element('aw-kinetics-result-file', {
  value: 'chosen.json', files: [{ size: 2, text: async () => '{}' }],
});
element('aw-kinetics-result-label');
VCS.workspace = {
  state: { project_id: 'project-a' },
  projects: [{ project_id: 'project-a', name: 'A' }],
};
VCS.call = async method => {
  trace.calls.push(method);
  return {
    ok: true, project_id: 'project-a', selected: false,
    result_sha256: 'invalid', confirmed_export_sha256: 'e'.repeat(64),
    latest_result_sha256: null, selection_revision: 0,
  };
};
loadAsset(process.argv[1]);
const seam = window.__VCS_ANALYSIS_TEST__;
seam.configure({ analysisId: 'kinetic-dashboard', projectId: 'project-a' });
const outcome = await seam.importKineticsResult({ target: fileInput });
assert.strictEqual(outcome, false);
assert.strictEqual(seam.snapshot().kinetics_uploaded_result, null);
assert.strictEqual(select.disabled, true);
assert.strictEqual(fileInput.value, '');
assert.ok(String(status.textContent).length > 0);
assert.deepStrictEqual(trace.calls, ['kinetics_result_import']);
""",
        str(ASSETS / "analysis-workbench.js"),
    )
