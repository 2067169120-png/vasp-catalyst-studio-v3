...
def _strict_authority(*, build: str = "vasp-6.4.3-build") -> dict:
    return {
        "schema": "vcstudio.scientific-fingerprint/v1",
        "status": "complete",
        "fingerprint_sha256": "1" * 64,
        "input_closure": {
            "schema": "vcstudio.scientific-input-closure/v1",
            "status": "complete",
            "component_sha256": "2" * 64,
        },
        "method_recipe": {
            "schema": "vcstudio.method-recipe/v1",
            "authority": "vcstudio.method-recipe-authority/v1",
            "semantic_sha256": "3" * 64,
        },
        "execution_environment": {
            "schema": "vcstudio.execution-environment/v1",
            "authority": "vcstudio.execution-environment-authority/v1",
            "engine": "vasp",
            "vasp_version": "6.4.3",
            "build_identity_sha256": hashlib.sha256(build.encode()).hexdigest(),
            "evidence_kind": "cluster-profile-attestation",
            "evidence_sha256": "4" * 64,
            "component_sha256": hashlib.sha256(
                f"environment:{build}".encode()).hexdigest(),
        },
        "missing": [],
        "integrity_issues": [],
        "binding_issues": [],
...
def test_neb_figure_requires_two_finite_relative_energy_points(tmp_path):
    spec = normalize_analysis_request(
        {"analysis_id": "neb-path"}, project_id=PROJECT)
    one_target = _neb_target(tmp_path / "one")
    for frame in ("01", "02"):
        (Path(one_target["path"]) / frame / "OSZICAR").write_text(
            "unparseable\n", encoding="utf-8")

    one = build_neb_analysis_view(
        spec, _neb_targets(one_target), method_evidence=_method)

    assert len(one["paths"][0]["points"]) == 3
    assert "neb_profile" not in one["figure_data"]
    assert one["report_binding"]["figures"] == []

    two_target = _neb_target(tmp_path / "two")
    (Path(two_target["path"]) / "01" / "OSZICAR").write_text(
        "unparseable\n", encoding="utf-8")
    two = build_neb_analysis_view(
        spec, _neb_targets(two_target), method_evidence=_method)
    assert sum(value is not None
               for value in two["figure_data"]["neb_profile"]["rel"]) == 2
    assert two["report_binding"]["figures"][0]["preset_key"] == "neb_profile"
...
def test_neb_main_completion_gate_fails_closed(
    ...
    if not keep_footer:
        ...
        (root / "vasprun.xml").write_text(
            "<modeling><calculation></calculation></modeling>\n",
            encoding="utf-8",
        )
    spec = normalize_analysis_request(
        {"analysis_id": "neb-path"}, project_id=PROJECT)
...
def test_neb_main_rejects_old_footer_followed_by_new_outcar_activity(tmp_path):
    target = _neb_target(tmp_path)
    root = Path(target["path"])
    (root / "OUTCAR").write_text(
        " General timing and accounting informations for this job:\n"
        " DAV: 1 -1.0 -0.1 0.01 1 0.1\n"
        " energy(sigma->0) = -9.0\n",
        encoding="utf-8",
    )
    (root / "vasprun.xml").write_text(
    ...
    assert path["completion_evidence"]["status"] == "unavailable"
    assert path["barriers"]["status"] == "unavailable"
    assert any("旧页脚" in issue for issue in path["barriers"]["blocking"])
...
def _convergence_targets(tmp_path: Path) -> list[dict]:
    ...
    for index, (encut, energy) in enumerate((
        ...
        target["method"] = {
            "status": "verified", "fingerprint": method,
            "strict_authority": _strict_authority(),
        }
...
@pytest.mark.parametrize("authority_key", [
    "input_closure", "method_recipe", "execution_environment",
])
def test_convergence_requires_every_strict_authority_component(
        tmp_path, authority_key):
    targets = _convergence_targets(tmp_path)
    targets[1]["method"]["strict_authority"].pop(authority_key)
    spec = normalize_analysis_request(
        {"analysis_id": "convergence-scan"}, project_id=PROJECT)
    ...
    assert any("authority is incomplete" in issue
               for issue in series["issues"])


def test_convergence_rejects_cross_build_platform_but_keeps_raw_points(tmp_path):
    targets = _convergence_targets(tmp_path)
    targets[2]["method"]["strict_authority"] = _strict_authority(
        build="vasp-6.4.3-other-build")
    spec = normalize_analysis_request(
        {"analysis_id": "convergence-scan"}, project_id=PROJECT)
    ...
    assert all(point["absolute_energy"]["value"] is not None
               for point in series["points"])
    assert series["platform"]["status"] == "unavailable"
    assert any("execution environments differ" in issue
               for issue in series["issues"])
    assert any("VASP build identities differ" in issue
               for issue in series["issues"])


def test_convergence_figure_omits_unrenderable_points(tmp_path):
    targets = _convergence_targets(tmp_path)
    (Path(targets[1]["path"]) / "INCAR").write_text(
        "EDIFF=1E-6\n", encoding="utf-8")
    spec = normalize_analysis_request(
        {"analysis_id": "convergence-scan"}, project_id=PROJECT)
    ...
    points = view["figure_data"]["convergence_curve"]["points"]
    assert len(points) == 3
    assert all(isinstance(point["x"], (int, float))
               and isinstance(point["energy"], (int, float)) for point in points)
    assert any(point["parameter"]["value"] is None
               for point in view["series"][0]["points"])


def test_convergence_figure_requires_two_finite_points(tmp_path):
    spec = normalize_analysis_request(
        {"analysis_id": "convergence-scan"}, project_id=PROJECT)
    one_targets = _convergence_targets(tmp_path / "one")
    for target in one_targets[1:]:
        (Path(target["path"]) / "INCAR").write_text(
            "EDIFF=1E-6\n", encoding="utf-8")
    one = build_convergence_analysis_view(
        spec, one_targets, method_evidence=lambda target: target["method"])
    assert len(one["series"][0]["points"]) == 4
    assert "convergence_curve" not in one["figure_data"]
    assert one["report_binding"]["figures"] == []

    two_targets = _convergence_targets(tmp_path / "two")
    for target in two_targets[2:]:
        (Path(target["path"]) / "INCAR").write_text(
            "EDIFF=1E-6\n", encoding="utf-8")
    two = build_convergence_analysis_view(
        spec, two_targets, method_evidence=lambda target: target["method"])
    assert len(two["figure_data"]["convergence_curve"]["points"]) == 2
    assert two["report_binding"]["figures"][0]["preset_key"] == "convergence_curve"
...
def test_convergence_final_snapshot_assert_covers_declared_closure_files(tmp_path):
    from vcstudio.project.analysis_sources import SourceSnapshotChanged

    targets = _convergence_targets(tmp_path)
    first = targets[0]
    root = Path(first["path"])
    dependency = root / "KPOINTS_OPT"
    dependency.write_text("extra scientific dependency\n", encoding="utf-8")
    first["manifest"]["inputs"]["input_closure"] = {
        "files": {"KPOINTS_OPT": _sha(dependency)},
    }
    _write_manifest(root, first["manifest"])
    changed = False

    def mutate_closure_after_capture(target, _snapshot):
        nonlocal changed
        if not changed:
            changed = True
    ...
    spec = normalize_analysis_request(
        {"analysis_id": "convergence-scan"}, project_id=PROJECT)
    with pytest.raises(SourceSnapshotChanged, match="KPOINTS_OPT changed"):
        build_convergence_analysis_view(
            spec, targets, method_evidence=mutate_closure_after_capture)
...
def test_production_method_resolver_rebuilds_bound_strict_authority(tmp_path):
    ...
    for index, target in enumerate(targets):
        ...
        manifest.update({"job_uuid": f"conv-authority-{index}",
                         "calc_type": "static"})
        manifest["inputs"].update({
            "engine": "vasp",
            "method_recipe": bind_method_recipe({
                "builder": "conv_scan", "point": index,
            }),
            "execution_environment": {
                "schema": EXECUTION_ENVIRONMENT_SCHEMA,
                "authority": EXECUTION_ENVIRONMENT_AUTHORITY,
                "engine": "vasp", "vasp_version": "6.4.3",
                "build_identity": "vasp-6.4.3-production-build",
                "evidence": {
    ...
    api_root.mkdir()
    api, _project_paths, _projects = _analysis_api(api_root)
    spec = normalize_analysis_request(
        {"analysis_id": "convergence-scan"}, project_id=PROJECT)

    view = build_convergence_analysis_view(
        spec, targets, method_evidence=api._analysis_workbench_method_evidence)

    assert view["series"][0]["platform"]["status"] == "available"
    authority = view["series"][0]["points"][0]["method"]["strict_authority"]
    assert authority["status"] == "complete"
    assert len(authority["execution_environment"]["build_identity_sha256"]) == 64
    rendered = json.dumps(view, ensure_ascii=False)
    assert "vasp-6.4.3-production-build" not in rendered
    assert str(tmp_path) not in rendered
    assert "components" not in authority
...
def _partial_aimd_target(root: Path, sample_count: int) -> dict:
    root.mkdir(parents=True)
    (root / "INCAR").write_text(
        "IBRION=0\nNSW=3\nPOTIM=1.0\nTEBEG=300\n", encoding="utf-8")
    (root / "POSCAR").write_text(_poscar(0.0), encoding="utf-8")
    (root / "OSZICAR").write_text("".join(
        f" {step} T= 300 E= {-10.0 + step / 1000:.6f} "
        "F= -10 E0= -10 EK= 0.1\n"
        for step in range(1, sample_count + 1)
    ), encoding="utf-8")
    xdat_lines = _xdatcar().splitlines()
    (root / "XDATCAR").write_text(
        "\n".join(xdat_lines[:7 + sample_count * 2]) + "\n", encoding="utf-8")
    manifest = {
        "state": "RUNNING", "task_type": "aimd",
        "inputs": {"potim_fs": 1.0, "steps": 3, "temp_k": 300.0},
    }
    _write_manifest(root, manifest)
    return _target(root, f"aimd-partial-{sample_count}", "aimd", manifest)


def test_aimd_figure_requires_two_finite_samples(tmp_path):
    spec = normalize_analysis_request(
        {"analysis_id": "aimd-diagnostics"}, project_id=PROJECT)
    one = build_aimd_analysis_view(
        spec, [_partial_aimd_target(tmp_path / "one", 1)], method_evidence=_method)
    assert len(one["trajectories"][0]["samples"]) == 1
    assert "aimd_diagnostic" not in one["figure_data"]
    assert one["report_binding"]["figures"] == []

    two = build_aimd_analysis_view(
        spec, [_partial_aimd_target(tmp_path / "two", 2)], method_evidence=_method)
    assert len(two["figure_data"]["aimd_diagnostic"]["steps"]) == 2
    assert two["report_binding"]["figures"][0]["preset_key"] == "aimd_diagnostic"
...
@pytest.mark.parametrize("bad_coordinate", ["0 0", "nan 0 0", "inf 0 0"])
def test_aimd_invalid_coordinate_rows_fail_closed_without_api_error(
        tmp_path, bad_coordinate):
    root = tmp_path / "aimd-invalid-coordinate"
    root.mkdir()
    (root / "INCAR").write_text(
        "IBRION=0\nNSW=3\nPOTIM=1.0\nTEBEG=300\n", encoding="utf-8")
    (root / "POSCAR").write_text(_poscar(0.0), encoding="utf-8")
    ...
    (root / "XDATCAR").write_text(
        _xdatcar().replace("0.0100 0 0", bad_coordinate), encoding="utf-8")
    manifest = {
        "state": "DONE", "task_type": "aimd",
        "inputs": {"potim_fs": 1.0, "steps": 3, "temp_k": 300.0},
    }
    _write_manifest(root, manifest)
    target = _target(root, "aimd-invalid-coordinate", "aimd", manifest)
    spec = normalize_analysis_request(
        {"analysis_id": "aimd-diagnostics"}, project_id=PROJECT)
    ...
    trajectory = view["trajectories"][0]
    assert trajectory["structure_diagnostics"]["status"] == "unavailable"
    assert "coordinate is invalid" in trajectory["structure_diagnostics"]["parser_error"]
    assert all(trajectory["metrics"][key]["value"] is None for key in (
        "trajectory_frames", "max_step_displacement", "final_rmsd", "max_rmsd"))
