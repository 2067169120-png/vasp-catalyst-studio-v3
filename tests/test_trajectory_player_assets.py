...
def test_player_uses_only_opaque_tokens_and_server_projected_scientific_fields():
    ...
    assert player.index('!frameTokens.owns(token)') < player.index(
        "VCS.call('trajectory_frame', token)")
    assert 'trajectoryReceiptMatches(frame, owner, token)' in player
...
def test_repair_confirmation_publishes_to_shared_operation_queue_and_reuses_key():
    ...
    assert 'requestFingerprint' in block
    assert 'trajectoryOwnerRow(ownerReceipt, jobId)' in block
    assert '自动路径仍最多 3 轮' in block
    assert '仅授权第 4 轮' in block
...
const fourthRound = new FakeElement();
seam.renderRepair(fourthRound, {
  ok: true, failure_class: 'NONCONVERGED', detected_evidence: 'bounded',
  suggested_change: 'continue', estimated_cost: { class: 'one_bounded_restart' },
  scientific_impact: 'unchanged', method_diff: [], execution_allowed: true,
  manual_round_limit_override: true,
}, () => {});
assert.ok(fourthRound.innerHTML.includes('automatic three-round cap has been reached'));
assert.ok(fourthRound.innerHTML.includes('authorize round 4'));
"""
...
def test_late_pages_and_unbound_frame_tokens_fail_owner_receipt_gate():
    player = _source('trajectory-player.js')
    assert 'createFrameTokenRegistry(owner)' in player
    assert 'frameTokens.clear();' in player
    assert 'frameTokens.bind(response, intent)' in player
    assert 'String(overview.job_id || \'\') !== requestedJobId' in player
    script = r"""
const fs = require('fs');
const vm = require('vm');
const assert = require('assert');
...
vm.runInThisContext(fs.readFileSync(process.argv[1], 'utf8'));
const seam = window.__VCS_TRAJECTORY_TEST__;
const owner = { sessionToken: 'session-a', sourceHash: 'source-a' };
const registry = seam.createFrameTokenRegistry(owner);
const gate = seam.createIntentGate();

const latePage = gate.begin();
registry.clear();
const currentPage = gate.begin();
assert.strictEqual(gate.valid(latePage), false);
assert.strictEqual(gate.valid(currentPage), true);
assert.strictEqual(registry.bind({
  session_token: 'session-a', source_hash: 'source-a',
  rows: [{ frame_token: 'frame-current' }],
}, currentPage), true);
assert.strictEqual(registry.owns('frame-current'), true);
assert.strictEqual(registry.owns('frame-injected'), false);

registry.clear();
assert.strictEqual(registry.owns('frame-current'), false);
assert.strictEqual(registry.bind({
  session_token: 'session-b', source_hash: 'source-a',
  rows: [{ frame_token: 'frame-foreign' }],
}, gate.begin()), false);
assert.strictEqual(registry.owns('frame-foreign'), false);
assert.strictEqual(seam.trajectoryReceiptMatches({
  session_token: 'session-a', source_hash: 'source-a', frame_token: 'frame-current',
}, owner, 'frame-current'), true);
assert.strictEqual(seam.trajectoryReceiptMatches({
  session_token: 'session-a', source_hash: 'source-a', frame_token: 'frame-foreign',
}, owner, 'frame-current'), false);
"""
    _run_node(script, str(ASSETS / 'trajectory-player.js'))
