...
def test_adsorption_provenance_lists_all_operands_and_requires_revalidated_report_binding(
    ...
    _rebuild(
        ...
        report_binding_resolver=legacy_report)
    legacy = service.live_provenance("project-1", job_id="job-config")
    assert not any(edge["type"] == "reported_in" for edge in legacy["edges"])

    def exact_binding(target):
        return {
            "schema": explorer_mod.REPORT_BINDING_SCHEMA,
            **legacy_report(target),
        }

    _rebuild(
        service, records, manifests, summaries=summaries,
        report_binding_resolver=exact_binding)
...
@pytest.mark.parametrize("collision", ["job_id", "source_id"])
def test_duplicate_project_job_or_source_identity_fails_closed(tmp_path, collision):
    records, manifests = _records(
        tmp_path, engines=("vasp",), statuses=("verified",))
    original = records[0]["project"]["members"]["configs"][0]
    duplicate = tmp_path / "project-1" / "config-duplicate"
    duplicate.mkdir()
    records[0]["project"]["members"]["configs"].append(str(duplicate))
    records[0]["project"]["config_species"][str(duplicate)] = "Li2S8"
    duplicate_manifest = copy.deepcopy(manifests[original])
    if collision == "job_id":
        duplicate_manifest["inputs"]["source_id"] = "source-distinct"
    else:
        duplicate_manifest["job_id"] = "job-distinct"
    manifests[str(duplicate)] = duplicate_manifest
