...
class _OwnedLedger:
    ...
    def register_owned(self, path, transaction_id):
        ...
        with self.lock:
            ...
            owned = current == transaction_id
            return {"added": owned, "preexisting": not owned, "owned": owned}

    def registration_state(self, path, transaction_id):
        value = str(Path(path).resolve())
        with self.lock:
            present = value in self.registrations
            owned = present and self.owners.get(value) == transaction_id
            return {"present": present, "owned": owned,
                    "preexisting": present and not owned}
    ...
    def release_registration_owner(self, path, transaction_id):
        value = str(Path(path).resolve())
        with self.lock:
    ...
    def ensure_registration_released(self, path, transaction_id):
        value = str(Path(path).resolve())
        with self.lock:
            current = self.owners.get(value)
            if current not in {None, transaction_id}:
                raise RuntimeError("owned by another transaction")
            was_present = value in self.registrations
            was_owned = current == transaction_id
            if not was_present:
                self.registrations.append(value)
            if was_owned:
                self.release_calls.append(Path(path).name)
...
def test_real_ledger_contract_maps_owner_and_recovers_release(tmp_path, monkeypatch):
    from vcstudio.cluster import ledger as cluster_ledger

    ledger_path = tmp_path / "real-ledger" / "jobs.json"
    monkeypatch.setattr(cluster_ledger, "default_ledger_path", lambda: ledger_path)
    original_ensure = cluster_ledger.ensure_registration_released
    release_attempts = 0

    def fail_first_release(path, transaction_id, **kwargs):
        nonlocal release_attempts
        release_attempts += 1
        if release_attempts == 1:
            raise OSError("simulated release interruption")
        return original_ensure(path, transaction_id, **kwargs)

    monkeypatch.setattr(
        cluster_ledger, "ensure_registration_released", fail_first_release)
    api, _sources, _registrations, _source, output = _api(
        tmp_path, ledger=cluster_ledger)
    confirmed = api.structure_source_confirm("1" * 32)["source_token"]
    dry_run = api.surface_dry_run(confirmed, {"miller": [1, 0, 0]}, {})
    destination = api.structure_output_select()
    ...
    assert partial["ok"] is False and partial["owner_release_pending"]
    payload = json.loads(ledger_path.read_text(encoding="utf-8"))
    stored_owners = payload["_transaction_owners"]
    assert len(stored_owners) == 1
    assert all(len(value) == 64 for value in stored_owners.values())

    monkeypatch.setattr(
        cluster_ledger, "ensure_registration_released", original_ensure)
    restarted, *_ = _api(tmp_path, ledger=cluster_ledger)
    ...
    assert recovered["ok"] is True and recovered["owner_release_pending"] == []
    final_payload = json.loads(ledger_path.read_text(encoding="utf-8"))
    assert "_transaction_owners" not in final_payload
    batch = next(path for path in output.iterdir() if path.is_dir())
    job = next(path for path in batch.iterdir() if path.is_dir())
    assert final_payload["job_dirs"] == [str(job.resolve())]


def test_complete_replay_repairs_missing_ledger_registration(tmp_path):
    ledger = _OwnedLedger()
    api, _sources, _registrations, _source, _output = _api(tmp_path, ledger=ledger)
    confirmed = api.structure_source_confirm("1" * 32)["source_token"]
    dry_run = api.surface_dry_run(confirmed, {"miller": [1, 0, 0]}, {})
    destination = api.structure_output_select()
    first = api.surface_create_candidates(
        dry_run["operation_token"], destination["output_token"])
    missing = ledger.registrations.pop()

    replay = api.surface_create_candidates(
        dry_run["operation_token"], destination["output_token"])

    assert first["ok"] is True
    assert replay["ok"] is True and replay["discoverability"] == "complete"
    assert ledger.registrations == [missing] and ledger.owners == {}

    ledger.registrations.clear()
    restarted, *_ = _api(tmp_path, ledger=ledger)
    restarted_destination = restarted.structure_output_select()
...
def test_complete_replay_fails_closed_for_another_registration_owner(tmp_path):
    ledger = _OwnedLedger()
    api, _sources, _registrations, _source, _output = _api(tmp_path, ledger=ledger)
    confirmed = api.structure_source_confirm("1" * 32)["source_token"]
    dry_run = api.surface_dry_run(confirmed, {"miller": [1, 0, 0]}, {})
    ...
    first = api.surface_create_candidates(
        dry_run["operation_token"], destination["output_token"])
    entry = ledger.registrations[0]
    ledger.owners[entry] = "f" * 32

    replay = api.surface_create_candidates(
        dry_run["operation_token"], destination["output_token"])

    assert first["ok"] is True
    assert replay["ok"] is False and replay["discoverability"] == "partial"
    assert replay["owner_release_pending"] == [first["jobs"][0]["job_id"]]
    assert ledger.owners[entry] == "f" * 32
