...
def test_configured_provider_without_current_projection_does_not_activate_card(
        tmp_path):
    api, paths, projects = _analysis_api(tmp_path)
    ...
    boot = api.analysis_workbench_bootstrap(project_id, "adsorption-energy")

    assert boot["ok"] is True
    card = next(item for item in boot["catalog"]["analyses"]
                if item["id"] == "free-energy-path")
    assert card["capability_status"] == "missing_prerequisite"
