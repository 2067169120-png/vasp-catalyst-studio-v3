...
REPORT_BINDING_SCHEMA = "vcstudio.frozen-analysis-binding/v1"
...
class ResearchIdentityError(ResearchExplorerError):
    """A project does not provide unique opaque job/source identities."""
...
def _bounded_payload_size(value: Any, *, label: str, maximum: int,
    ...
    nodes = 0
    entries = 0
    entry_limit = maximum if max_entries is None else max_entries
    ...
    while stack:
        ...
        if entries > entry_limit:
            raise ResearchIndexLimitError(f"{label} exceeds the entry limit")
    return total
...
class ResearchIndexService:
        ...
        for record in (() if registry_blocked else records):
            ...
            try:
                ...
                for member in _member_records(project):
                    ...
                    if job_id in seen_job_ids or source_id in seen_source_ids:
                        raise ResearchIdentityError(
                            "project contains duplicate opaque job/source identities")
            ...
            except ResearchIdentityError:
                failures.append({
                    "project_ref": project_id,
                    "code": "duplicate_job_or_source_identity",
                })
    ...
    def _report_binding_projection(value: Any,
        ...
        value = value if isinstance(value, Mapping) else {}
        required = {
            "schema", "revision_id", "current", "frozen_graph_revalidated",
            "bound_job_ids", "bound_analyses",
        }
        if set(value) != required or value.get("schema") != REPORT_BINDING_SCHEMA:
            return {}
        revision = _safe_label(value.get("revision_id"), maximum=128)
        if (not revision or not _OPAQUE_RE.fullmatch(revision)
                or value.get("current") is not True
                or value.get("frozen_graph_revalidated") is not True):
            return {}
        raw_jobs = value.get("bound_job_ids")
        raw_analyses = value.get("bound_analyses")
        if (not isinstance(raw_jobs, list) or not raw_jobs
                or len(raw_jobs) > MAX_PROJECT_MEMBERS
                or not isinstance(raw_analyses, list) or not raw_analyses
                or len(raw_analyses) > MAX_PROJECT_MEMBERS):
            return {}
        if any(
                not _OPAQUE_RE.fullmatch(str(item))
                or _PATH_RE.search(str(item)) or _SECRET_RE.search(str(item))
                for item in raw_jobs):
            return {}
        bound_jobs = {str(item) for item in raw_jobs}
        if len(bound_jobs) != len(raw_jobs):
            return {}
        valid: list[dict[str, str]] = []
        seen_analyses: set[tuple[str, str, str]] = set()
        analysis_fields = {
            "job_id", "source_id", "quantity", "energy_contract_id",
            "quantity_sha256",
        }
        for raw in raw_analyses:
            ...
            energy_contract_id = str(raw.get("energy_contract_id") or "")
            quantity_sha256 = str(raw.get("quantity_sha256") or "")
            identity = (job_id, source_id, quantity)
            if (quantity not in {"energy_eV", "barrier_eV"}
                    or identity in seen_analyses
                    or not _OPAQUE_RE.fullmatch(job_id)
                    or not _OPAQUE_RE.fullmatch(source_id)
                    or job_id not in bound_jobs
                    or not re.fullmatch(r"[0-9a-f]{64}", quantity_sha256)
                    or (quantity == "energy_eV"
                        and not _OPAQUE_RE.fullmatch(energy_contract_id))
                    or (quantity == "barrier_eV" and energy_contract_id)):
                return {}
            seen_analyses.add(identity)
            match = next((entry for entry in entries if (
                str(entry.get("job_id") or "") == job_id
