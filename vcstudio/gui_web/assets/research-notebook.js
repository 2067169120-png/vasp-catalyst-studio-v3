(function () {
  ...
  const RECORD_ID_RE = /^rn-[a-f0-9]{32}$/;
  ...
  const COPY = {
    ...
    'research_notebook.runtime.replayed': ['已确认先前操作的稳定 receipt；未重复追加。',
      'The stable receipt for the prior operation was confirmed; nothing was appended twice.'],
  ...
  function requestFingerprint(value) {
    ...
    return `${left.toString(16).padStart(8, '0')}${right.toString(16).padStart(8, '0')}-${text.length}`;
  }

  function newOperationKey() {
    const cryptoApi = window.crypto;
    if (cryptoApi && typeof cryptoApi.randomUUID === 'function') {
      return `notebook-operation.${cryptoApi.randomUUID()}`;
    }
    if (cryptoApi && typeof cryptoApi.getRandomValues === 'function') {
      const bytes = new Uint8Array(16); cryptoApi.getRandomValues(bytes);
      return `notebook-operation.${Array.from(bytes)
        .map(value => value.toString(16).padStart(2, '0')).join('')}`;
    }
    operationNonce += 1;
    return `notebook-operation.${Date.now().toString(16)}-${operationNonce.toString(16)}`;
  }

  function operationKey(root, action, payload) {
    const fingerprint = requestFingerprint({
      project_id: State.projectId, action, payload,
    });
    const current = root.__rnOperation;
    if (current && current.projectId === State.projectId
        && current.action === action && current.fingerprint === fingerprint) {
      return current.idempotencyKey;
    }
    const idempotencyKey = newOperationKey();
    root.__rnOperation = {
      schema: 'vcstudio.research-notebook-operation-ref/v1',
      projectId: State.projectId, action, fingerprint, idempotencyKey,
    };
    return idempotencyKey;
  }

  function clearOperation(root, idempotencyKey = '') {
    const current = root.__rnOperation;
    if (!current || !idempotencyKey || current.idempotencyKey === idempotencyKey) {
      delete root.__rnOperation;
    }
  }
  ...
  function receiptMatches(result, guard, idempotencyKey) {
    const receipt = result && result.operation_receipt;
    if (!receipt || receipt.idempotency_key !== idempotencyKey
        || !SHA256_RE.test(String(receipt.request_digest || ''))
        || !SHA256_RE.test(String(receipt.record_digest || ''))
        || !RECORD_ID_RE.test(String(receipt.record_id || ''))
        || !Number.isInteger(receipt.revision) || receipt.revision < 1
        || typeof receipt.replayed !== 'boolean'
        || result.created_record_id !== receipt.record_id
        || Number(result.revision) < receipt.revision
        || Number(result.revision) < guard.revision) return false;
    if (!receipt.replayed && receipt.revision !== guard.revision + 1) return false;
    const record = (result.records || []).find(item =>
      item.record_id === receipt.record_id && item.revision === receipt.revision);
    return !!record && String(record.record_digest || '') === receipt.record_digest;
  }

  function applyView(result, root, successMessage, guard, idempotencyKey) {
    ...
    if (safeId(result.project_id) !== guard.projectId
        ...
        || !receiptMatches(result, guard, idempotencyKey)) return false;
    State.view = result;
    const message = result.operation_receipt.replayed
      ? tr('research_notebook.runtime.replayed') : successMessage;
    clearCompose(root); renderAll(); setStatus(root, message, 'success'); return true;
  }
  ...
  async function tombstoneRecord(root, recordId) {
    ...
    const idempotencyKey = operationKey(root, 'tombstone', {
      record_id: recordId, reason, actor,
    });
    setBusy(true);
    try {
      const result = await VCS.call('research_notebook_tombstone',
        guard.projectId, recordId, reason, actor, expectedGuard(guard), idempotencyKey);
  ...
  async function load(root) {
    ...
    const rebound = State.projectId !== projectId;
    State.projectId = projectId;
    if (rebound) { State.view = null; renderAll(); }
    setBusy(true);
    ...
    try {
      ...
      if (result && result.ok === true && (safeId(result.project_id) !== projectId
          || !SHA256_RE.test(String(result.project_identity_digest || ''))
          || !Number.isInteger(result.revision) || result.revision < 0
          || (result.head_digest != null
            && !SHA256_RE.test(String(result.head_digest))))) {
        State.view = null; renderAll(); return false;
      }
      State.view = result && result.ok === true ? result : null; renderAll();
  ...
  if (window.__VCS_TEST__) {
    window.ResearchNotebook.__test = {
      ...
      expectedGuard, pickAttachments, requestFingerprint, operationKey,
      receiptMatches, clearOperation,
      tombstoneRecord,
    };
  }
