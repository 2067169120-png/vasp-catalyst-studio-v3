...
(function () {
  ...
  const PUBLIC_ERROR_CODES = new Set([
    ...
    'invalid_token', 'keyring_unavailable', 'network_busy', 'network_confirmation_required',
    'network_disabled', 'network_failure', 'network_session_changed', 'partial_data',
    'project_unavailable', 'provider_error', 'provider_unavailable', 'rate_limited',
    'request_timeout', 'response_too_large', 'result_too_large', 'schema_drift',
    'structure_changed', 'structure_unavailable', 'token_replayed', 'unknown_provider',
  ]);
  function errorMessage(result, fallback) {
    const error = plain(result && result.error); const code = safeId(error.code);
    const generic = tr('reference.error.unavailable', '外部参考暂不可用。');
    const safeFallback = String(fallback || generic);
    if (!PUBLIC_ERROR_CODES.has(code)) return safeFallback;
    return tr(`reference.error.${code}`, safeFallback);
  }
