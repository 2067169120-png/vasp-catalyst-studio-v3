...
class Api:
    ...
    def __init__(self, *, profiles_mod=None, secrets_mod=None, ssh_test_mod=None,
                 ...
                 workspace_context_store=None, reaction_domain_source=None):
    ...
    def _project_report_contracts(self, proj, project_path, summary, fed, *,
        ...
        input_fingerprint = self._bind_reaction_projection_fingerprint(
            project_fingerprint, reaction_projection_sha256)
    ...
    # ── Phase C:统一报告工作台服务接线 ─────────────────────────────────────
    @staticmethod
    def _bind_reaction_projection_fingerprint(base, reaction_projection_sha256):
        reaction_hash = str(reaction_projection_sha256 or '').strip().lower()
        if not reaction_hash:
            return base
        return hashlib.sha256(json.dumps({
            'project_fingerprint': base,
            'reaction_projection_sha256': reaction_hash,
    ...
    def _current_reaction_projection_sha256(self, project, workspace_project_id):
        ...
        return str(view.get('source_projection_sha256') or '').strip().lower()
    ...
    def _report_workbench_build(self, path, report_spec, work_dir):
        ...
        return {
            ...
            'reaction_projection_sha256': reaction_projection_sha256,
            'report_model_sha256': report_model_sha256,
            'contracts': contracts,
            'model': model,
    ...
    def _report_workbench_current_state(self, path, report_spec=None):
        ...
        if projection is not None:
            ...
            reaction_projection_sha256 = str(
                reaction_view.get('source_projection_sha256') or '')
    ...
    def _report_workbench_persist_build(self, build, rendered, *, revision):
        ...
        return self._persist_report_marker(
            ...
            expected_reaction_projection_sha256=build.get(
                'reaction_projection_sha256'),
    ...
    def _analysis_workbench_capability_cards(self, context, projects, selected_view):
        ...
        states = {
            ...
            'free-energy-path': (
                'available' if self._reaction_domain_source is not None or mode == 'lis'
                else 'mode_mismatch'),
    ...
    def _persist_report_marker(self, project, summary, files, *, kind='final',
        ...
        with self._report_marker_lock:
            ...
            current_reaction_projection_sha256 = ''
            expected_reaction_hash = str(
                expected_reaction_projection_sha256 or '').strip().lower()
            if expected_reaction_hash:
                if not str(workspace_project_id or '').strip():
                    raise _ReportInputChanged(
                        '报告已生成但缺少 reaction projection 项目身份，未登记产物')
                current_reaction_projection_sha256 = (
                    self._current_reaction_projection_sha256(
                        current_project, workspace_project_id))
                if current_reaction_projection_sha256 != expected_reaction_hash:
                    raise _ReportInputChanged(
            ...
            marker = {
                ...
                'reaction_projection_sha256': current_reaction_projection_sha256,
                'revision': self._json_safe_report_result(revision or {}),
            }
    ...
    def _report_marker_current_reaction_hash(self, project, marker):
        expected = str(
            (marker or {}).get('reaction_projection_sha256') or '').strip().lower()
        if not expected:
            return ''
        workspace_project_id = str(
            (marker or {}).get('workspace_project_id') or '').strip()
        ...
        current = self._current_reaction_projection_sha256(
    ...
    def _report_marker_current(self, project, summary=None) -> bool:
        ...
        try:
            ...
            if scope_error:
                return False
            reaction_hash = self._report_marker_current_reaction_hash(project, marker)
